...
[查看程式碼 ➜](10.Pointers%20as%20Function%20Returns%20in%20C/realloc-usage.c)

---
### 🔍 realloc 的運作方式
| 名稱              | 說明 |
| ----------------- | ---- |
| `realloc(ptr, new_size)` | 嘗試在 **原位址** 調整記憶體大小，若空間不足，會配置新空間並複製原資料 |
| `ptr`             | 原本的指標（必須是 `malloc` / `calloc` / `realloc` 配置的） |
| 回傳值             | 可能與 `ptr` 相同（原地調整成功）或不同（搬到新位址） |
...
```
並且 `realloc` 失敗，`ptr` 原本的位址會遺失，造成記憶體洩漏
...
| 語法                           | 等價於            | 用途說明                   |
| ---------------------------- | -------------- | ---------------------- |
| `realloc(ptr, larger_size)`  | 放大             | 例：`n * 2`，若原空間不足會搬到新位置 |
| `realloc(ptr, smaller_size)` | 縮小             | 例：`n / 2`，可能釋放尾端空間     |
| `realloc(ptr, 0)`            | `free(ptr)`    | 釋放記憶體                  |
| `realloc(NULL, size)`        | `malloc(size)` | 新配置記憶體                 |

---
### ✅ 安全習慣
- 放大/縮小後，只使用新回傳的指標

- 若需要保留原資料且防止洩漏，先用暫存變數接回傳值

- 縮小後不要讀取超過新範圍的內容（未定義行為）
