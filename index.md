...
這個網站是針對 [Pointers in C/C++ (YouTube)](https://www.youtube.com/watch?v=zuegQmMdy8M) 影片所做的學習筆記。
...
- [7. Array as function argument](#7-array-as-function-argument)
- [8. Character arrays and pointers - part 1](#8-character-arrays-and-pointers---part-1)
  - [字串基本概念](#字串基本概念)
  - [字串與指標](#字串與指標)
  - [用字元指標將字串傳入函式](#用字元指標將字串傳入函式)
- [9. Character arrays and pointers - part 2](#9-character-arrays-and-pointers---part-2)
...
- [12. Pointers and dynamic memory - stack vs heap](#12-pointers-and-dynamic-memory---stack-vs-heap)
  ...
  - [Heap 是什麼？該如何使用它？](#heap-是什麼該如何使用它)
...
- [17. Memory Leak in C/C++](#17-memory-leak-in-cc)
...
## 最終總結 — C 語言 Pointer 思維地圖

在 C 語言中，**Pointer（指標**）是**位址的變數**，它承載的不只是「資料在哪裡」，更是讓我們能在不同記憶體區域（Stack / Heap / Global / Code）之間靈活操作的工具。
要真正掌握它，不只是死背語法，而是養成一套思考模式：

---

### 1️⃣ 先搞懂記憶體分區與生命週期
- **Stack**：函式內部的區域變數、參數，生命週期隨函式結束自動釋放

- **Heap**：動態配置（`malloc/new`），生命週期由你決定，必須手動釋放（`free/delete`）

- **Global/Static**：整個程式執行期間都存在

- **Code（Text）**：儲存機器碼指令，不可修改
...
### 2️⃣ 指標的核心：它儲存的是位址
- `int *p` → `p` 裡存的是「**某個 int 在記憶體的位址**」

- `*p` → 透過這個位址取到裡面的值

- 陣列變數、字串常量、函式名稱，都能在特定情況下自動退化成指標（位址）
...
### 3️⃣ 函式與指標的互動
- **Call by Value**：傳值複製，函式內改變不影響外部

- **Call by Address / Reference**：傳位址，函式內可直接改外部資料

陣列傳進函式會退化成指標，必須額外傳長度
...
### 4️⃣ 堆疊與堆積的取捨
- Stack：快、安全、不需釋放，但空間有限

- Heap：彈性大、適合大量或動態資料，但必須手動管理
...
- 可將函式位址存進指標，動態決定執行哪段邏輯（Callback、事件處理、演算法選擇）
...
### 6️⃣ 防禦性思維 — 避免踩雷
- 動態配置後一定要釋放（避免 Memory Leak）

- 釋放後指標設成 NULL（避免懸空指標 Dangling Pointer）

- 不要回傳指向 Stack 區域變數的指標

- 操作指標前先確定它不是空的（`if (p != NULL)`）

- 避免越界存取（陣列、指標運算時特別注意）
---

## 🎯 最後一句話記住：
**Pointer 就是位址，位址背後是記憶體，記憶體背後是生命週期。**

**只要先想清楚「它在哪裡、活多久、誰負責回收」，你就能用指標駕馭 C 語言。**
